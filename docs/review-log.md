...
| 2026-08-16 | `proof/usage-exceeded-notice` | Live failure reporting | Live deployment | Pending | 0 | 0 | Scratch head that proves the deployed failure notice names exhausted usage and is rewritten on retry. First head was cancelled before the provider answered, so a second head repeats the probe. |
